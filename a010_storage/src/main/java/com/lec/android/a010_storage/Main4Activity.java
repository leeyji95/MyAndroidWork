...
// 기존의 JDBC 프로그래밍과는 다른 독특한 메소드 제공..   대놓고 insert() 등의 메소드가 따로 정의되어 있다.
public class Main4Activity extends AppCompatActivity {
    ...
    void insert(String name, int age, String address) {
        ...
        // 리턴값은 auto-generated key 값 리턴
        long result = db.insert(tableName, null, values); // 특정 테이블에 이러한 값을 넣는다.  지금 SQL 구문 작성 안함.
        // long 정수값으로 리턴함으로 long 타입으로 담아준다.xxxxxxxxx
        //  무조건 1개 리턴. 실패하면 -1 리턴
        // 두번째 매개변수 ? -> 아래 주석 확인

        Log.d("myapp", "번째 row INSERT 성공");
    ...
    void select() {
        ...
        imm.hideSoftInputFromWindow(getCurrentFocus().getWindowToken(), 0); // 공식처럼 쓰기
    } // end select()
    ...
    void update(String name, int age, String etAddress) {
        ...
        values.put("address", etAddress);   // 변경할 값
...
만약에 value 값이 null인 경우,
모두 null일 경우, 하나라도 null 넣어라. 의미. 아예 비워두지 말고.
/
 */
