...
// 사용자 앨범에 있는 이미지 보여주기 할 건데,
// 이 이미지를 가져오려면 꼭  사용자 권한 받아야한다. 그걸 받는 코드 작성 필요
// 주소, 통화내역 등 모두 개인정보에 해당. 이런걸 안드로인드에서는 위험권한 요청이라고 하는데,  이건 런타임에서 요청함.
// 실행시점에서 요청하도록 되어 있따. 모든 권한이 아니라, 위험한 권한 그룹(캘린더, 전화관련, 카메라, 문자, 위치 , 이미지 등 저장소(storagd)등등) -> 반드시 런타임때 요청받아야한다.
// 위험권한 획득을 해야 접근 가능.]
...
public class Main3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 여기까지 등장시키고~ 메니페스트에 권한 요청하고 오기


        // 버튼 클릭 시  파일경로에서 이미지 받아오고 그걸  화면에 띄우고
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
                ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) // 이 코드가 가동중일때 이 코드가 읽히는지
                        != PackageManager.PERMISSION_GRANTED) {  // PackageManager.PERMISSION_GRANTED(권한승인)  권한을 승인 받지 않았을 때 권한 요청을 수행한다는 뜻
