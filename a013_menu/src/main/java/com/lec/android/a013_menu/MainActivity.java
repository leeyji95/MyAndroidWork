...
public class MainActivity extends AppCompatActivity {
    ...
    // '옵션 메뉴' 버튼이 '처음' 눌러졌을 때 실행되는 메소드
    // 메뉴버튼을 눌렀을 떄 보여줄 menu 에 대해서 정의
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        // 이때 add() ,   MenuItem 리턴
        MenuItem item1 = menu.add(1, MENUITEM_YELLOW, 100, "노랑");  // 주어진 메뉴에 추가
        MenuItem item2 = menu.add(1, MENUITEM_ORANGE, 100, "오렌지");  // 주어진 메뉴에 추가
        MenuItem item3 = menu.add(1, MENUITEM_CYAN, 100, "파랑");  // 주어진 메뉴에 추가


        return true;     // flase 리턴하면 메뉴는 보여지지 않는다.
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        Log.d("myapp", "onCreateOptionsMenu - 옵션메뉴가 화면에 보여질때 마다 호출됨");

        // 메뉴가 보여질때마다 '로그인;, ;로그아웃 이 enable/disable 토글되어 보이기
        if(blog){ // 로그인 상태
            // getItem(index_ index 번째 MenuItem 객체 리턴
    ...
    public void showInfo(MenuItem item){
        ...
        String title = item.getTitle().toString();  // 옵션메뉴 아이템의 titile 값
        int groupOd = item.getGroupId(); // 옵션 메뉴 아이템의 그룹 아이디
        int order = item.getOrder(); // 메뉴 아이템의 순번

        String msg = "id:"+ id + "title:" + title + "groupId: " + groupOd + " order:" + order;
        Log.d("myapp", msg);
        Toast.makeText(this, title + "메뉴클릭", Toast.LENGTH_LONG).show();
