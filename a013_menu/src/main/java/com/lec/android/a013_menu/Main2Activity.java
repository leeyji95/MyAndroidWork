...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 코드로 register, unregister 할 수 있다는 걸 보여줌
        // 꾹 누르면 Context 메뉴 나옴
        registerForContextMenu(tvCtxMenu); //뷰객체(위젯 등...) 에 ContextMenu 를  등록함
        // unregisterForContextMenu(tvCtxMenu);    // 등록된 ContextMenu 제거
    ...
    // onCreateContextMenu()
    // 컨텍스트 메뉴가 생성될때마다 호출되는 콜백 메서드
    @Override
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        super.onCreateContextMenu(menu, v, menuInfo);
        Log.d("myapp", "onCreateContextMenu()");

        // 각각의 뷰(위젯) View v 마다 다르게 메뉴 작동하도록 설정 가능 .. 어느 위치에서 우클릭 했는가. 이것이 두번째 매개변수로 넘어옴
        switch (v.getId()){
            case R.id.tvCtxMenu:
// 몇번 그룹의 몇 번 id
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        Log.d("myapp", "onContextItemSelecte()");
        showInfo(item);
            ...
            case 1:
                switch (id) {
                    case 1: // 빨강
                        ...
                        return true;
                    case 2: // 녹색
                        ivCtxMenu.setImageResource(R.drawable.face17);
                        return true;
                    case 3: // 파랑
