...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Vibrator vibrator
                = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE); // 하드웨어에 있는 어떤 특정 서비스? 가져올 떄
        ...
        btnVib2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                long [] pattern = {100, 300, 100, 700, 300, 2000}; // 단위 ms // 롱타입의 배열로 패턴을 준비한다!
                        //        대기,진동, 대기, 진동...
                        // 짝수 인덱스 : 대기
                        // 홀수 인덱스 : 진동
                   vibrator.vibrate(pattern, // 진동패턴( 배열)
                           -1); // 반복
                // 0 : 무한반복, -1 : 반복 없음
                // 양의 정수 : 진동패턴 배열의 해당 인덱스부터 진동 무한 반복.  ==> pattern 배열의 인덱스 번호
            }
        });
